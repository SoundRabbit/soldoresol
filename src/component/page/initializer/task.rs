...
async fn initialize_object_store(
    ...
    if !has_client {
        database = unwrap!(idb::create_object_store(&database, "client").await; None);
    }
    if !has_rooms {
        database = unwrap!(idb::create_object_store(&database, "rooms").await; None);
    }
    if !has_resources {
        database = unwrap!(idb::create_object_store(&database, "resources").await; None);
    }
    if !has_tables {
        database = unwrap!(idb::create_object_store(&database, "tables").await; None);
    }
    if !has_characters {
        database = unwrap!(idb::create_object_store(&database, "characters").await; None);
    }
