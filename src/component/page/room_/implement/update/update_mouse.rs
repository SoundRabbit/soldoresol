use super::{
    block, BlockId, BoxblockTool, CharacterTool, Implement, ObjectId, PenTool, PointlightTool,
...
impl Implement {
    ...
    fn mouse_position_in_craftboard(
        &self,
        craftboard_id: &BlockId,
        position_in_canvas: &[f32; 2],
    ) -> Option<[f32; 2]> {
        self.block_arena
            .map(craftboard_id, |craftboard: &block::Craftboard| {
                let p = craftboard.position();
                let a = self.camera_matrix.collision_point_with_z(
                    &self.canvas_size,
                    self.mouse_state.cursor().now().position_in_canvas(),
                    p[2],
                );
                [a[0] - p[0], a[1] - p[1]]
            })
    ...
    fn last_mouse_position_in_craftboard(&self, craftboard_id: &BlockId) -> Option<[f32; 2]> {
        self.mouse_position_in_craftboard(
            craftboard_id,
            self.mouse_state.cursor().last().position_in_canvas(),
        )
    }

    fn now_mouse_position_in_craftboard(&self, craftboard_id: &BlockId) -> Option<[f32; 2]> {
        self.mouse_position_in_craftboard(
            craftboard_id,
            self.mouse_state.cursor().now().position_in_canvas(),
        )
    ...
    fn drawing_texture_id(&self, craftboard_id: &BlockId) -> Option<BlockId> {
        self.block_arena
            .chain(&craftboard_id, |craftboard: &block::Craftboard| {
                Some(craftboard.layer_group_id())
            })
            .and_then(|_, layer_group: &block::LayerGroup| {
                layer_group
                    .selected_layer()
                    .and_then(|layer| layer.as_drawing())
                    .or(layer_group
                        .layers()
                        .rev()
                        .find_map(|layer| layer.as_drawing()))
                    .map(|drawing| BlockId::clone(drawing.drawing_texture_id()))
            })
    }

    fn drawed_texture_id(&self, craftboard_id: &BlockId) -> Option<BlockId> {
        self.block_arena
            .chain(&craftboard_id, |craftboard: &block::Craftboard| {
                Some(craftboard.layer_group_id())
            })
    ...
    fn draw_as_pen(&mut self, a: &[f64; 2], b: &[f64; 2], )
    ...
    fn update_tabletool_pen(&mut self) {
        ...
        let last_craftboard_id = self.mouse_state.cursor().last().craftboard_id();
        let now_craftboard_id = self.mouse_state.cursor().now().craftboard_id();

        if self.mouse_state.primary_btn().is_dragging() {
            if last_craftboard_id != now_craftboard_id {
                self.finish_drawing_line(last_craftboard_id, pen);
            }
            if let Some(drawing_texture_id) = self.drawing_texture_id(now_craftboard_id) {
                let cursor = self.mouse_state.cursor();
                let a = cursor.last().position_in_craftboard().now();
                let b = cursor.now().position_in_craftboard().now();

                self.local_block_arena.map_mut(
                    &drawing_texture_id,
                    |texture: &mut block::Texture| {
    ...
    fn finish_drawing_line(&mut self, craftboard_id: &BlockId, pen: &PenTool) {
        if let Some((a, drawing_texture_id)) = join_some!(
            self.now_mouse_position_in_craftboard(craftboard_id),
            self.drawed_texture_id(craftboard_id)
        ) {
            self.block_arena
                .map(&drawing_texture_id, |texture: &block::Texture| {
                    let a = texture.texture_position(&[a[0] as f64, a[1] as f64]);
                    self.drawing_line.push([a[0] as f64, a[1] as f64]);
                });
        }
        let mut points = self
            .drawing_line
        ...
        if let Some((drawing_texture_id, drawed_texture_id)) = join_some!(
            self.drawing_texture_id(craftboard_id),
            self.drawed_texture_id(craftboard_id)
        ) {
            self.local_block_arena.map_mut(
                &drawing_texture_id,
    ...
    fn update_tabletool_shape_line(&mut self) {
        ...
        if self.mouse_state.primary_btn().is_dragging() {
            let drawing_craftborad_id = self.mouse_state.primary_btn().drag_start().craftboard_id();
            if let Some((drawing_texture_id, a, b)) = join_some!(
                self.drawing_texture_id(drawing_craftborad_id),
                self.mouse_position_in_craftboard(
                    &drawing_craftborad_id,
                    self.mouse_state
                        .primary_btn()
                        .drag_start()
                        .position_in_canvas(),
            ...
            let drawing_craftborad_id = self.mouse_state.primary_btn().drag_start().craftboard_id();
            if let Some((drawing_texture_id, drawed_texture_id, a, b)) = join_some!(
                self.drawing_texture_id(drawing_craftborad_id),
                self.drawed_texture_id(drawing_craftborad_id),
                self.mouse_position_in_craftboard(
                    &drawing_craftborad_id,
                    self.mouse_state
                        .primary_btn()
